...
<template>
  <div class="crawler-container">
    ...
    <div class="crawler-header">
      <h1>🕷️ 网络信息爬取系统</h1>
      <p class="subtitle">支持多平台数据采集与智能分析（支持回车键快速触发）</p>
    </div>
          ...
          <el-form :model="baiduParams" label-width="120px">
            ...
            <el-form-item label="结果数量">
              <el-input-number 
                ...
                :max="50"
                placeholder="爬取结果数量"
                @change="checkCrawlCount('baidu', baiduParams.count)"
              />
          ...
          <el-form :model="bilibiliParams" label-width="120px">
            ...
            <el-form-item label="结果数量">
              <el-input-number 
                ...
                :max="50"
                placeholder="爬取结果数量"
                @change="checkCrawlCount('bilibili', bilibiliParams.count)"
              />
          ...
          <el-form :model="csdnParams" label-width="120px">
            ...
            <el-form-item label="结果数量">
              <el-input-number 
                ...
                :max="50"
                placeholder="爬取结果数量"
                @change="checkCrawlCount('csdn', csdnParams.count)"
              />
      ...
      <div v-if="results.length > 0" class="results-section">
        ...
        <div class="results-stats">
          <div class="results-stats-header">
            <el-statistic title="关键词" :value="currentKeyword || '未设置'" />
            <el-statistic title="总结果数" :value="results.length" />
            <el-statistic title="执行时间" :value="executionTime" suffix="ms" />
            <!-- 自定义URL爬取不显示成功率 -->
            <el-statistic v-if="selectedSpider !== 'url'" title="成功率" :value="successRate" suffix="%" />
          </div>
        </div>
        ...
        <div class="results-list">
          ...
          >
            ...
            <div v-if="expandedResults.includes((currentPage - 1) * pageSize + index) && !isBaiduResult(result)" class="result-detail">
              <!-- B站结果展开内容 -->
              <div v-if="isBilibiliResult(result)" class="bilibili-detail">
...
export default {
  ...
  setup() {
    ...
    /**
     * 执行爬取任务
     * 根据选中的爬虫类型和用户输入的参数执行爬取
     * 修复：使用max_results参数替代count，支持真实的结果数量控制
     */
    const executeCrawl = async () => {
      ...
      if (selectedSpider.value !== 'url') {
        ...
        switch (selectedSpider.value) {
          case 'baidu':
            keyword = baiduParams.keyword?.trim()
            break
          case 'bilibili':
            keyword = bilibiliParams.keyword?.trim()
            break
          case 'csdn':
            keyword = csdnParams.keyword?.trim()
            break
        }
      ...
      // 验证自定义URL是否为空
      if (selectedSpider.value === 'url' && !urlParams.url?.trim()) {
      ...
      try {
        ...
        switch (selectedSpider.value) {
          case 'baidu':
            params = { keyword: baiduParams.keyword, max_results: baiduParams.count }
            break
          case 'bilibili':
            params = { keyword: bilibiliParams.keyword, max_results: bilibiliParams.count }
            break
          case 'csdn':
            params = { keyword: csdnParams.keyword, max_results: csdnParams.count }
            break
          case 'url':
            params = { ...urlParams }
            break
        ...
        if (response.data.success) {
          let crawlResults = response.data.results || []
          // 按指标排序结果
          crawlResults = sortResultsByMetrics(crawlResults)
          results.value = crawlResults
          executionTime.value = Date.now() - startTime
          // 自定义URL爬取结果默认展开，其他爬虫默认收起
          if (selectedSpider.value === 'url') {
          ...
          // 爬取完成后更新成功率
          updateSuccessRate()
          ElMessage.success(`爬取完成！获取到 ${results.value.length} 条结果`)
        } else {
