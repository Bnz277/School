...
def scan_code(session2):
    ...
    try:
        ...
        # 保存 Cookie 为字符串
        cookies_str = "; ".join([f"{cookie.name}={cookie.value}" for cookie in session2.cookies])
        with open(temp_cookie_file, 'w', encoding='utf-8') as f:
            f.write(cookies_str)

        bili_jct = findall(r'bili_jct=(.*?)(;|$)', cookies_str)[0][0]
...
def verification():
    try:
        ...
        if resp1['data']['isLogin']:
            ...
            v1.set(status)
            print_cookies(login_session)  # 打印当前 Cookie
        else:
            thread_it(bz_login)
        label_code.config(image=tk_image)
    except Exception as e:
...
if __name__ == '__main__':
    ...
    try:
        with open(temp_cookie_file, 'r', encoding='utf-8') as f:
            bzcookie = f.read()
            bili_jct = findall(r'bili_jct=(.*?)(;|$)', bzcookie)[0][0]
    except Exception as e:
